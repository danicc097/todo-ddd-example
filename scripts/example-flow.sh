...
echo -e "\n=== REGISTER ALICE ==="
ALICE_RES=$(./todo-cli register -d -p '{"email": "alice-'"$SEED"'@example.com", "name": "Alice", "password": "Password123!"}' --idempotency-key "$(seeded_uuid "alice-reg")" | get_json)
ALICE_ID=$(echo "$ALICE_RES" | jq -r .id)
echo "Alice ID: $ALICE_ID"

echo -e "\n=== LOGIN ALICE ==="
LOGIN_RES=$(./todo-cli login -d -p '{"email": "alice-'"$SEED"'@example.com", "password": "Password123!"}' | get_json)
export API_TOKEN=$(echo "$LOGIN_RES" | jq -r .accessToken)

echo -e "\n=== CREATE WORKSPACE ==="
WS_RES=$(./todo-cli onboard-workspace -d -p '{"name": "Alice HQ", "description": "Top Secret"}' --idempotency-key "$(seeded_uuid "alice-ws")" | get_json)
WS_ID=$(echo "$WS_RES" | jq -r .id)
echo "Workspace ID: $WS_ID"

echo -e "\n=== CREATE TODO ==="
TODO_RES=$(./todo-cli create-todo -d -p '{"title": "Deploy to production"}' --idempotency-key "$(seeded_uuid "todo-1")" | get_json)
TODO_ID=$(echo "$TODO_RES" | jq -r .id)
echo "Todo ID: $TODO_ID"

echo -e "\n=== CREATE TAG ==="
TAG_RES=$(./todo-cli create-tag "$WS_ID" -d -p '{"name": "critical-'"${SEED:0:10}"'"}' --idempotency-key "$(seeded_uuid "tag-crit")" | get_json)
...
echo -e "\n=== REGISTER BOB ==="
BOB_RES=$(./todo-cli register-user -d -p '{"email": "bob-'"$SEED"'@example.com", "name": "Bob"}' --idempotency-key "$(seeded_uuid "bob-reg")" | get_json)
...
echo "The above should have failed"

echo -e "\n===  INITIATE TOTP ==="
TOTP_RES=$(./todo-cli initiate-totp -d | get_json)
...
echo -e "\n===  VERIFY TOTP CODE ==="
VERIFY_RES=$(./todo-cli verify-totp -d -p "{\"code\": \"$CODE\"}" | get_json)
